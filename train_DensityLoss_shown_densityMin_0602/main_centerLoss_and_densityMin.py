...
def train(model, criterion_xent, criterion_cent,
    ...
    if args.plot:
        ...
        density_cnter_prob = [1] * num_classes
        density_fea = [[0, 0]] * num_classes
    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        for i in range(prob_train.size(0)): #batch_size = x.size(0)
            if prob_train[i, labels[i]].data.cpu().numpy() < density_cnter_prob[labels[i]]:
                density_cnter_prob[labels[i]] = prob_train[i, labels[i]].data.cpu().numpy()
                density_fea[labels[i]] = features[i,:].data.cpu().numpy()
    ...
    # Append the Density Centers at the last $2*num_classes$
    all_features.append(density_fea)
...
def plot_features(features, labels, num_classes, epoch, prefix):
    ...
    # Plot Density Centers with Larger Markers
    for label_idx in range(num_classes):
        plt.scatter(
            features[label_idx - 2*num_classes, 0],
            features[label_idx - 2*num_classes, 1],
            c='black',
            s=5,
        )
    ...
    for label_idx in range(num_classes):
        plt.scatter(
            features[label_idx-num_classes, 0],
            features[label_idx-num_classes, 1],
            c='white',
            s=8,
        )
